...
/**
 * Performance test for the DeliveryPerson entity.
 */
class DeliveryPersonGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DeliveryPerson entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all deliveryPeople")
            .get("/api/delivery-people")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new deliveryPerson")
            .post("/api/delivery-people")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "iD":"0"
                , "name":"SAMPLE_TEXT"
                , "surname":"SAMPLE_TEXT"
                , "phone":"SAMPLE_TEXT"
                , "vehiculeType":"SAMPLE_TEXT"
                , "latitude":"0"
                , "longitude":"0"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_deliveryPerson_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created deliveryPerson")
                .get("${new_deliveryPerson_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created deliveryPerson")
            .delete("${new_deliveryPerson_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
