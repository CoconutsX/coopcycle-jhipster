...
describe('Cooperative e2e test', () => {
  ...
  const cooperativeSample = { iD: 47014, address: 'Computer' };
  ...
  describe('new Cooperative page', () => {
    ...
    it('should create an instance of Cooperative', () => {
      cy.get(`[data-cy="iD"]`).type('24897').should('have.value', '24897');

      cy.get(`[data-cy="name"]`).type('circuit').should('have.value', 'circuit');

      cy.get(`[data-cy="phone"]`).type('+33 376073').should('have.value', '+33 376073');

      cy.get(`[data-cy="address"]`).type('a override').should('have.value', 'a override');

      cy.get(`[data-cy="webURL"]`).type('non-volatile de optical').should('have.value', 'non-volatile de optical');
