...
import info4.gl.dm.coopcycle.service.OrderService;
...
/**
 * REST controller for managing {@link info4.gl.dm.coopcycle.domain.Order}.
 */
@RestController
@RequestMapping("/api")
public class OrderResource {
     ...
     */
    @GetMapping("/orders")
    public Mono<List<OrderDTO>> getAllOrders(@RequestParam(required = false) String filter) {
        if ("course-is-null".equals(filter)) {
            log.debug("REST request to get all Orders where course is null");
            return orderService.findAllWhereCourseIsNull().collectList();
        }

        if ("payment-is-null".equals(filter)) {
            log.debug("REST request to get all Orders where payment is null");
            return orderService.findAllWherePaymentIsNull().collectList();
        }
